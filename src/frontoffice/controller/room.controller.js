...
export const createRoom = async (req, res) => {
  try {
    ...
    const floor = await Floor.findOne({
      where: { floor_no: validated.floor_no },
    });

    if (!floor) return res.sendError("Invalid floor_no", 400);
...
export const getAllRooms = async (req, res) => {
  try {
    ...
    const where = {};
    if (query.floor_no) where.floor_no = query.floor_no;
